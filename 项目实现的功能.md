...
- 播放一个视频、切换到另一个视频、退出VideoPage均会进行记录当前进度
...
- 在需要使用的组件中import后，通过调用Toast.方法名即可生成各种不同的toast

- 共有info、success、warning、error、loading、noAni这6种toast

- 可参数有：需要显示的信息、是否显示遮盖、toast持续时间、toast结束时的回调、图标类型
...
- 若未登录而是通过在地址栏输入"/me"进入的，则跳转到登录页面
...
- 均可在header中点击编辑后，进行删除

- 支持全选/反选，全选、删除按钮也会随着是否全选中而相应地变化
...
- 收藏夹可新建、编辑
  ...
  - 编辑时各表单的默认值为用户当前所设置的值

  - 编辑完成后返回上一页时显示的是编辑后的信息

- 收藏夹列表可折叠

- 收藏夹明细

  - 根据是否为本人的收藏夹，是的话header才显示省略号

  - 省略号点开后可执行：删除、编辑、批量操作、一键清空失效内容
...
- 搜索功能可选，将把搜索关键字传递给使用HeaderWithTools的组件

- 最右侧按钮共有5中模式：0：无；1：省略号；2：编辑；3：加号；4：自定义

- 省略号模式会弹出底部操作区域，并添加遮盖层

- 部操作区域可选操作为：编辑信息、批量管理、一键清除失效内容、删除
...
- 弹幕分为滑动和固定两种
  ...
  - 固定弹幕会依次排列，屏幕高度铺满前不会重叠

- 发送弹幕

  - 登录后才可发送弹幕,未登录则提示跳转登录,编辑时会显示编辑中

  - 可设置颜色，位置，大小

  - 发送后会即时显示在视频中，暂停时会等视频播放后才显示

  - 关闭后再打开时之前输入的内容会保留,如果是发送成功后再打开则会清空

  - 发送失败会有提示

- 暂停弹幕

  - 视频暂停时弹幕也会暂停，并保留其当前位置，倒计时

  - 可通过点击单独暂停某个弹幕从而进行操作，其他弹幕的状态完全不受影响
