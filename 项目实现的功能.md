...
移动可以通过scrollLeft实现,但由于其为web api而非css,transition无法对其奏效,只能使用递归或定时器模拟动画

但该api最大的问题是每次滚动均从头滚动，就会产生tabBar先回到最左边再向右移动的糟糕的动画效果

#### 拖到头后会继续左移

放弃scroll api改用translate3d后,tabbar到右边拖到头后会继续左移,导致tabbar最右端脱离右边界

解决办法时添加判断条件，如果一个向右拖动将到头，则只拖动可以拖动的最大距离。由于没有offsetRight这个api,故只能变相用另外几个属性相减求offsetRight
