...
3. 获取验证结果validate与seccode，以继续进行短信或密码登录
...
2. 加密登录密码
  ...
  - 获取RSA公钥key与盐值hash（盐值有效时间为20s）
  
  - 连接盐值+密码字串，使用JSEncrypt对拼接盐值后的密码进行RSA公钥加密，得到base64格式密文

3. 登录操作验证

  - 使用账号username（手机号或邮箱）+密文密码password+登录密钥key+极验challenge+验证结果validate+验证结果seccode

  - 后端验证成功后，response headers中会包含4个set-cookie，客户端成功设置这4个cookie后即完成登录
