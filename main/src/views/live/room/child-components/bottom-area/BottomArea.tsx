...
function BottomArea(props: BottomAreaProps, ref) {
  const { description } = props;
  const [index, setIndex] = useState(0);
  const chatRef: React.RefObject<HTMLDivElement> = useRef(null);
  const leftTabRef: React.RefObject<HTMLDivElement> = useRef(null);
  const rightTabRef: React.RefObject<HTMLDivElement> = useRef(null);
  ...
  useImperativeHandle(ref, () => ({
    ...
    sendMsg: msg => {
      ...
      setTimeout(() => {
        ...
        if (chatDOM.scrollHeight > Math.ceil(chatDOM.scrollTop) + chatDOM.clientHeight) {
          needScroll = true;
        }
  ...
  useEffect(() => {
    ...
    setInterval(() => {
      ...
      if (children.length > 1000) {
        ...
        subChildren.forEach(child => { chatDOM.removeChild(child); });
      }
    }, 5000);

    // 如果消息中包含gift数据，则将它赋给gifts
    getRoomGifts().then(result => { if (result.code === "1") { gifts = result.data; } });

    leftTabRef.current.addEventListener("click", () => { setIndex(0); })
    rightTabRef.current.addEventListener("click", () => { setIndex(1); })
  }, []);
...
export default forwardRef(BottomArea);